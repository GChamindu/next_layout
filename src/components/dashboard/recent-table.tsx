...
// Example columns (customize per page)
export const recentColumns: ColumnDef<RecentItem>[] = [
  { accessorKey: 'item', header: 'Item' },
  { accessorKey: 'status', header: 'Status', cell: ({ row }) => (
    <Badge variant={row.original.status === 'success' ? 'default' : row.original.status === 'pending' ? 'secondary' : 'destructive'}>
      {row.original.status}
    </Badge>
  )},
